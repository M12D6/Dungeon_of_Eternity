// ═══════════════════════════════════════════════════════════════
//  DUNGEON OF ETERNITY — Expanded Edition
//  A deep roguelike with factions, pets, crafting, housing,
//  achievements, skill trees, subclasses, and 100+ hours of content.
// ═══════════════════════════════════════════════════════════════
const { useState, useEffect, useRef, useCallback, useMemo } = React;

// ═══════════════════════════════════════════════════════════════
// BUNDLED DATA & SYSTEMS (originally separate modules)
// ═══════════════════════════════════════════════════════════════
...
// Dungeon-tier based rarity — epic+ is genuinely rare in early dungeons
function rollRarityForDungeon(dungeonTier = 1, luck = 0) {
    // Base weights per dungeon tier (1-8)
    ...
    // Luck bonus shifts weight from common → rare/epic
...
const DIFFICULTIES = {
    EASY: { name: "Easy", icon: "🟢", levelOffset: 0, statMult: 0.7, lootMult: 0.5, expMult: 0.8, color: "#22c55e" },
    NORMAL: { name: "Normal", icon: "🟡", levelOffset: 7, statMult: 1.0, lootMult: 1.0, expMult: 1.0, color: "#eab308" },
    HARD: { name: "Hard", icon: "🟠", levelOffset: 14, statMult: 1.5, lootMult: 1.5, expMult: 1.5, color: "#f97316" },
    INSANE: { name: "Insane", icon: "🔴", levelOffset: 21, statMult: 2.5, lootMult: 2.5, expMult: 2.5, color: "#ef4444" },
    NIGHTMARE: { name: "Nightmare", icon: "💀", levelOffset: 29, statMult: 5.0, lootMult: 5.0, expMult: 5.0, color: "#a855f7" },
};
function getDungeonDifficultyBrackets(dungeon) {
    // Base progression: Starter is 1, next is 10, etc.
    // We bridge them by adding difficulty offsets
...
// Legacy alias for non-dungeon contexts
...
// ═══════════════════════════════════════════════════════════════
//  DUNGEONS — 8 Named Dungeons (Dungeon Quest style)
// ═══════════════════════════════════════════════════════════════
const DUNGEONS = [
    {
        id: "sewer_depths",
...
const ELEMENTAL_ENCHANTMENTS = {
    ASTRAL: { 
        name: "Astral", 
        icon: "🌌", 
        color: "#8b5cf6", 
        desc: "60% more gold, 40% more exp, tons of consumables. SUCH A BLESSING!",
        goldMult: 1.6,
        expMult: 1.4,
        consumableBonus: 3,
        duration: "permanent"
    ...
    BLAZING: { 
        ...
        icon: "🔥", 
        color: "#ef4444", 
        desc: "20% more gold, 30% more exp, Burn status on next mob. Mob can't adapt!",
        goldMult: 1.2,
        expMult: 1.3,
        burnOnNextHit: true,
    ...
    BLUSTER: { 
        ...
        icon: "⚡", 
        color: "#3b82f6", 
        desc: "20% gold, 50% more exp, Explosion on m1's. Wears off after kill!",
        goldMult: 1.2,
        expMult: 1.5,
        explosionOnHit: true,
        duration: "next_kill"
    ...
    FROST: { 
        ...
        icon: "❄️", 
        color: "#06b6d4", 
        desc: "30% gold, 40% exp, Freeze chance on attacks. Slows enemies!",
        goldMult: 1.3,
        expMult: 1.4,
        freezeChance: 0.25,
        duration: "next_kill"
    ...
    SHADOW: { 
        ...
        icon: "🌑", 
        color: "#6b7280", 
        desc: "40% gold, 20% exp, Invisibility on first hit. Guaranteed crit!",
        goldMult: 1.4,
        expMult: 1.2,
        invisNextHit: true,
        duration: "next_kill"
    ...
    EARTH: { 
        ...
        icon: "🟫", 
        color: "#92400e", 
        desc: "50% gold, 30% exp, Thorns damage. Reflects damage back!",
        goldMult: 1.5,
        expMult: 1.3,
        thorns: 0.3,
        duration: "next_kill"
...
const CLASSES = {
    WARRIOR: {
        ...
        subclasses: {
            JUGGERNAUT: {
                name: "Juggernaut", icon: "🛡️", desc: "Immovable tank. Halves damage below 50% HP.",
                stats: { hp: 50, def: 15, atk: -3 }, spells: ["Unbreakable", "Taunt", "Stonewall"]
            },
            GLADIATOR: {
                name: "Gladiator", icon: "🗡️", desc: "Dual-wield crit maniac. +15% crit, +20% ATK.",
                stats: { atk: 12, crit: 15, spd: 3 }, spells: ["Dual Strike", "Arena Roar", "Coliseum"]
            },
            WARLORD: {
                name: "Warlord", icon: "👑", desc: "Commander of war. Passive buffs allies/self.",
    ...
    MAGE: {
        ...
        subclasses: {
            ARCHMAGE: {
                name: "Archmage", icon: "🌟", desc: "Pure arcane. +40% spell damage, +100 max mana.",
                stats: { mana: 100, crit: 8 }, spells: ["Arcane Explosion", "Time Warp", "Mana Storm"]
            },
            PYROMANCER: {
                name: "Pyromancer", icon: "🔥", desc: "Fire specialist. All fire spells burn + 30% stronger.",
                stats: { mana: 60, atk: 4 }, spells: ["Phoenix Dive", "Volcano", "Immolate"]
            },
            CRYOMANCER: {
                name: "Cryomancer", icon: "❄️", desc: "Ice lord. Freeze chance doubled, +DEF from cold armor.",
    ...
    ROGUE: {
        ...
        subclasses: {
            ASSASSIN: {
                name: "Assassin", icon: "🎯", desc: "Guaranteed crit from stealth. +50% crit damage.",
                stats: { crit: 20, atk: 8 }, spells: ["Shadow Kill", "Vanish", "Execute"]
            },
            DUELIST: {
                name: "Duelist", icon: "⚔️", desc: "Parry master. 35% to reflect melee attacks.",
                stats: { spd: 6, def: 8 }, spells: ["Riposte", "Blade Dance", "Flurry"]
            },
            TRICKSTER: {
                name: "Trickster", icon: "🎭", desc: "Chaos agent. Randomly curses, poisons, or charms.",
    ...
    PALADIN: {
        ...
        subclasses: {
            CRUSADER: {
                name: "Crusader", icon: "⚔️", desc: "Holy warrior. +40% vs. undead/demons, smites crit.",
                stats: { atk: 10, hp: 30 }, spells: ["Crusader's Charge", "Sacred Strike", "Holy Avenger"]
            },
            TEMPLAR: {
                name: "Templar", icon: "🛡️", desc: "Divine tank. +Healing effectiveness, +DEF.",
                stats: { def: 12, hp: 60 }, spells: ["Lay on Hands", "Aegis", "Guardian Angel"]
            },
            INQUISITOR: {
                name: "Inquisitor", icon: "🔥", desc: "Zealot. Hunts heretics, ignores resistances.",
    ...
    NECROMANCER: {
        ...
        subclasses: {
            SUMMONER: {
                name: "Summoner", icon: "👥", desc: "Commands minions. Can have 3 active.",
                stats: { mana: 80 }, spells: ["Summon Skeleton", "Summon Ghoul", "Army of Bones"]
            },
            BLOODMAGE: {
                name: "Blood Mage", icon: "🩸", desc: "Spends HP as mana. +50% damage per HP missing %.",
                stats: { hp: 40, mana: -40 }, spells: ["Blood Sacrifice", "Hemorrhage", "Crimson Bath"]
            },
            DEATHKNIGHT: {
                name: "Death Knight", icon: "🗡️", desc: "Melee necro. Heals from kills, wields two-handers.",
    ...
    BERSERKER: {
        ...
        subclasses: {
            WARLOCK: {
                name: "Warlock", icon: "🔥", desc: "Rage-casting hybrid. Attacks buff spell dmg.",
                stats: { mana: 100, crit: 5 }, spells: ["Demon Fire", "Pact", "Chaos Bolt"]
            },
            REAVER: {
                name: "Reaver", icon: "⚡", desc: "Speed berserker. Extra turn after kill.",
                stats: { spd: 8, atk: 4 }, spells: ["Blitz", "Bloodrush", "Slaughter"]
            },
            TITAN: {
                name: "Titan", icon: "🗿", desc: "Colossal. HP doubled, all damage +20%.",
    ...
    RANGER: {
        ...
        subclasses: {
            MARKSMAN: {
                name: "Marksman", icon: "🎯", desc: "Sniper specialist. +50% crit dmg, +range.",
                stats: { crit: 15, atk: 8 }, spells: ["Headshot", "Ballista", "Kill Shot"]
            },
            BEASTMASTER: {
                name: "Beastmaster", icon: "🐺", desc: "Tames pets. Companion fights alongside you.",
                stats: { hp: 30, mana: 30 }, spells: ["Call Beast", "Pack Tactics", "Wild Charge"]
            },
            TRAPPER: {
                name: "Trapper", icon: "🪤", desc: "Master of traps. Stack 3 traps per fight.",
    ...
    SORCERER: {
        ...
        subclasses: {
            ELEMENTALIST: {
                name: "Elementalist", icon: "🌪️", desc: "All elements. Elemental combos trigger bonus dmg.",
                stats: { mana: 50 }, spells: ["Elemental Fury", "Prismatic Blast", "Maelstrom"]
            },
            STORMCALLER: {
                name: "Stormcaller", icon: "⛈️", desc: "Lightning mastery. Chains on crit.",
                stats: { spd: 4, crit: 10 }, spells: ["Lightning Surge", "Storm Lord", "Tempest"]
            },
            VOIDCALLER: {
                name: "Voidcaller", icon: "🌀", desc: "Void specialist. Ignores all resist.",
...
const ALL_ITEMS = [
    ...
    // ═══════════════ ARMOR (100+) ═══════════════
    A("a_rags", "Tattered Rags", "COMMON", 5, "LIGHT", "👕", 5, { hp: 10 }),
    A("a_leather", "Leather Vest", "COMMON", 10, "LIGHT", "🧥", 30, { hp: 20 }),
    A("a_padded", "Padded Gambeson", "COMMON", 12, "LIGHT", "🧥", 40, { hp: 25 }),
    A("a_travel", "Traveler's Cloak", "COMMON", 8, "LIGHT", "🧥", 28, { hp: 15, mana: 10 }),
    A("a_chain", "Chain Mail", "UNCOMMON", 20, "MEDIUM", "🛡️", 90, { hp: 30 }),
    A("a_ringmail", "Ring Mail", "UNCOMMON", 18, "MEDIUM", "🛡️", 85, { hp: 25 }),
    A("a_studded", "Studded Leather", "UNCOMMON", 16, "LIGHT", "🧥", 80, { hp: 20, spd: 2 }),
    A("a_mage_robe", "Mage Robe", "UNCOMMON", 12, "LIGHT", "👘", 90, { mana: 50 }),
    A("a_iron", "Iron Plate", "RARE", 35, "HEAVY", "🛡️", 250, { hp: 40 }),
    A("a_bone", "Bone Armor", "RARE", 30, "MEDIUM", "🦴", 260, { hp: 50, mana: 10 }),
    A("a_elven", "Elven Silk", "RARE", 22, "LIGHT", "👘", 270, { hp: 30, mana: 40, spd: 3 }),
    A("a_plate", "Knight's Plate", "RARE", 40, "HEAVY", "🛡️", 280, { hp: 45 }),
    A("a_spikedarmor", "Spiked Armor", "RARE", 38, "HEAVY", "⚔️", 285, { hp: 40, unique: "thorns_5" }),
    A("a_shadow", "Shadow Plate", "EPIC", 50, "HEAVY", "🌑", 700, { hp: 60, atk: 5 }),
    A("a_mithril", "Mithril Armor", "EPIC", 60, "MEDIUM", "🛡️", 750, { hp: 80 }),
    A("a_flame_mail", "Flame Mail", "EPIC", 52, "MEDIUM", "🔥", 720, { hp: 70, elem: "fire" }),
    A("a_frost_mail", "Frost Mail", "EPIC", 54, "MEDIUM", "❄️", 720, { hp: 70, elem: "ice" }),
    A("a_light_robe", "Light Robe", "EPIC", 38, "LIGHT", "✨", 700, { hp: 50, mana: 90, elem: "holy" }),
    A("a_dragon", "Dragonscale Mail", "LEGENDARY", 85, "HEAVY", "🐉", 2200, { hp: 120, unique: "fire_resist" }),
    A("a_divine", "Divine Plate", "LEGENDARY", 95, "HEAVY", "✨", 2800, { hp: 150, mana: 30, atk: 5, unique: "holy_ward" }),
    A("a_necro", "Necroplate", "LEGENDARY", 80, "HEAVY", "💀", 2300, { hp: 130, mana: 60 }),
    A("a_royal", "Royal Guard Plate", "LEGENDARY", 100, "HEAVY", "👑", 2700, { hp: 160, unique: "kings_ward" }),
    A("a_titan", "Titan's Shell", "LEGENDARY", 110, "HEAVY", "🗿", 3000, { hp: 200 }),
    A("a_void", "Void Aegis", "MYTHIC", 130, "HEAVY", "🌀", 9999, { hp: 200, mana: 50, atk: 10, unique: "void_ward" }),
    A("a_god_armor", "Raiment of Gods", "MYTHIC", 150, "LIGHT", "🌟", 9999, { hp: 300, mana: 100, atk: 15, unique: "divine" }),
    A("a_phoenix_mail", "Phoenix Mail", "UNIQUE", 70, "MEDIUM", "🔥", 2200, { hp: 100, unique: "rebirth" }),
    A("a_thorn_armor", "Thornmail", "UNIQUE", 65, "HEAVY", "🌹", 1800, { hp: 80, unique: "thorns_20" }),

    // ═══════════════ HELMETS ═══════════════
    H("h_cap", "Cloth Cap", "COMMON", 4, "LIGHT", "🎩", 10, { hp: 10, mana: 5 }),
    H("h_cowl", "Dark Cowl", "COMMON", 5, "LIGHT", "🧢", 15, { hp: 8 }),
    H("h_hood", "Leather Hood", "COMMON", 6, "LIGHT", "👒", 18, { hp: 12 }),
    H("h_iron", "Iron Helm", "UNCOMMON", 12, "HEAVY", "⛑️", 70, { hp: 15 }),
    H("h_wizard", "Wizard Hat", "UNCOMMON", 6, "LIGHT", "🎩", 75, { hp: 10, mana: 30 }),
    H("h_helm_knight", "Knight Helm", "UNCOMMON", 15, "HEAVY", "⛑️", 80, { hp: 20 }),
    H("h_horned", "Horned Helm", "RARE", 20, "HEAVY", "⛑️", 220, { hp: 20, atk: 3 }),
    H("h_circlet", "Silver Circlet", "RARE", 12, "LIGHT", "👑", 200, { mana: 60, hp: 25 }),
    H("h_visored", "Visored Helm", "RARE", 22, "HEAVY", "⛑️", 230, { hp: 25 }),
    H("h_crown", "Crown of Thorns", "EPIC", 30, "LIGHT", "👑", 680, { hp: 40, mana: 20, atk: 5 }),
    H("h_dragon", "Dragon Crown", "LEGENDARY", 45, "LIGHT", "🐉", 2100, { hp: 70, mana: 40, atk: 8 }),
    H("h_godcrown", "Crown of Gods", "MYTHIC", 70, "LIGHT", "👑", 9999, { hp: 120, mana: 80, atk: 15 }),
    H("h_archmage", "Archmage Hood", "EPIC", 25, "LIGHT", "🎩", 650, { hp: 30, mana: 100 }),
    H("h_skullhelm", "Skull Helmet", "UNIQUE", 35, "MEDIUM", "💀", 1500, { hp: 50, unique: "fear_aura" }),

    // ═══════════════ BOOTS ═══════════════
    B_("b_sandals", "Worn Sandals", "COMMON", 3, "LIGHT", "👟", 8, { hp: 5 }),
    B_("b_leather", "Leather Boots", "UNCOMMON", 8, "LIGHT", "👢", 60, { hp: 10 }),
    B_("b_travel", "Traveler's Boots", "UNCOMMON", 9, "LIGHT", "👢", 65, { hp: 12, spd: 2 }),
    B_("b_iron", "Iron Greaves", "RARE", 16, "HEAVY", "👢", 210, { hp: 15 }),
    B_("b_shadow", "Shadowstep Boots", "EPIC", 22, "LIGHT", "👟", 660, { hp: 20, atk: 4, spd: 4 }),
    B_("b_winged", "Winged Boots", "EPIC", 25, "LIGHT", "🪶", 700, { hp: 30, mana: 20, spd: 6 }),
    B_("b_eternal", "Boots of Eternity", "LEGENDARY", 40, "LIGHT", "✨", 2000, { hp: 60, mana: 30, atk: 5 }),
    B_("b_cosmic", "Cosmic Treads", "MYTHIC", 65, "LIGHT", "🌌", 9999, { hp: 100, mana: 50, atk: 10 }),
    B_("b_stealth", "Silent Boots", "UNIQUE", 30, "LIGHT", "👟", 1500, { spd: 10, unique: "stealth_gain" }),
    B_("b_gravityless", "Gravityless", "UNIQUE", 28, "LIGHT", "🪶", 1400, { spd: 15, unique: "no_trap" }),

    // ═══════════════ GLOVES ═══════════════
    G_("g_leather", "Leather Gloves", "COMMON", 4, "LIGHT", "🧤", 20, { atk: 2 }),
    G_("g_mail", "Mailed Gloves", "UNCOMMON", 9, "MEDIUM", "🧤", 70, { atk: 4 }),
    G_("g_iron", "Iron Gauntlets", "RARE", 16, "HEAVY", "🧤", 220, { atk: 6, hp: 15 }),
    G_("g_shadow", "Shadow Gloves", "EPIC", 22, "LIGHT", "🧤", 680, { atk: 10, crit: 8 }),
    G_("g_dragon", "Dragon Gloves", "LEGENDARY", 40, "LIGHT", "🧤", 2100, { atk: 15, crit: 12 }),
    G_("g_void", "Void Touch", "MYTHIC", 60, "LIGHT", "🧤", 9999, { atk: 20, crit: 20 }),
    G_("g_thief", "Thief's Gloves", "UNIQUE", 20, "LIGHT", "🧤", 1200, { spd: 8, unique: "gold_find" }),

    // ═══════════════ SHIELDS / OFFHAND ═══════════════
    S_("s_buckler", "Wooden Buckler", "COMMON", 8, "LIGHT", "🛡️", 30, { hp: 10 }),
    S_("s_kite", "Kite Shield", "UNCOMMON", 18, "MEDIUM", "🛡️", 110, { hp: 20 }),
    S_("s_tower", "Tower Shield", "RARE", 32, "HEAVY", "🛡️", 290, { hp: 35 }),
    S_("s_aegis", "Aegis", "EPIC", 50, "MEDIUM", "🛡️", 750, { hp: 60, unique: "block_chance" }),
    S_("s_dragonsh", "Dragon Shield", "LEGENDARY", 70, "HEAVY", "🐉", 2400, { hp: 100, unique: "fire_resist" }),
    S_("s_voidshield", "Void Shield", "MYTHIC", 100, "HEAVY", "🌀", 9999, { hp: 150, unique: "void_ward" }),

    // ═══════════════ CAPES ═══════════════
    C_("c_cloak", "Cloth Cloak", "COMMON", 3, "LIGHT", "🧣", 15, { hp: 8 }),
    C_("c_mantle", "Wolf Mantle", "UNCOMMON", 7, "LIGHT", "🧣", 65, { hp: 15, atk: 2 }),
    C_("c_ranger", "Ranger's Cloak", "RARE", 15, "LIGHT", "🧣", 230, { hp: 25, spd: 4 }),
    C_("c_void", "Void Mantle", "EPIC", 25, "LIGHT", "🧣", 690, { hp: 40, mana: 50 }),
    C_("c_hero", "Hero's Cape", "LEGENDARY", 40, "LIGHT", "🧣", 2200, { hp: 70, atk: 8, mana: 30 }),
...
function generateEnemy(level, biome, dungeonTier = 1, isBoss = false, diff = "EASY") {
    ...
    // Assign element based on biome or enemy type
    const elementTypes = ["FIRE", "ICE", "LIGHTNING", "DARK", "HOLY", "EARTH"];
    const elementMap = {
        "ember_caverns": "FIRE",
        "glacial_fortress": "ICE", 
        "haunted_catacombs": "DARK",
        "necropolis": "DARK",
        "sunken_forest": "EARTH",
        "divine_citadel": "HOLY",
        "abyss_of_shadows": "DARK"
    };
    const enemyElement = elementMap[biome] || elementTypes[Math.floor(Math.random() * elementTypes.length)];

    // Check if enemy should be enchanted (15% chance)
    const isEnchanted = !isBoss && Math.random() < 0.15;
    const enchantmentMultiplier = isEnchanted ? 0.6 : 1.0;
    const expMultiplier = isEnchanted ? 1.67 : 1.0;
    ...
    return {
        ...
        id,
        name: isEnchanted ? `✨${ed.name}✨` : ed.name,
        icon: isEnchanted ? "✨" : ed.icon,
        level,
        maxHp: Math.floor(hpBase * diffMult * enchantmentMultiplier),
        hp: Math.floor(hpBase * diffMult * enchantmentMultiplier),
        atk: Math.floor(atkBase * diffMult * enchantmentMultiplier),
        def: Math.floor(defBase * diffMult * enchantmentMultiplier),
        exp: Math.floor(((level * 15) + (isBoss ? 200 : 20)) * dData.expMult * expMultiplier),
...
// Helper for banded stat scaling
...
// Calculates a single basic-attack outcome via the Layered Pipeline
...
function calcSpellDamage(attacker, defender, spellName, mods = {}) {
    ...
    // 4. Combo Layer (Max 4 stacks)
    const comboStacks = Math.min(4, attacker.comboStacks || 0);
    const comboMult = 1 + (comboStacks * 0.25);
    dmg *= comboMult;

    // 5. Mitigation (C-Constant)
    const def = defender.totalDef || defender.def || 0;
    const mitigation = 1 - (def / (def + C_CONSTANT));
    dmg *= mitigation;
    ...
    // 7. Criticals (Hard capped at 50% chance)
    const critChance = Math.min(50, (attacker.crit || 10) + (mods.critBonus || 0));
    const isCrit = Math.random() * 100 < critChance;
    if (isCrit) dmg *= (1.5 + (attacker.critDmgBonus || 0));

    // 8. Adaptive Layer (Chaos Engine)
    const adaptiveRes = defender.adaptiveRes?.[sp.type] || 0;
    dmg *= (1 - adaptiveRes);
    if (adaptiveRes > 0) mods.isAdapted = true;

    // 9. Execution Cap (35% Enemy Max HP)
    const finalDmg = Math.max(3, Math.floor(Math.min(dmg, (defender.maxHp || 100) * DAMAGE_CAP_PCT)));
...
function generateMap(floor, biomeId = null, opts = {}) {
    ...
    // Place features
    const chests = Math.min(2, Math.floor(Math.random() * 3));
...
// Apply random stat variance (±variance) to item stats
...
function generateDungeonLoot(dungeonTier = 1, floor = 1, count = 1, opts = {}) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (!base) continue;
        // Scale base stats by dungeon tier
...
// Legacy generateLoot wrapper (used by non-dungeon contexts like secrets, portals)
...
// Upgrade a weapon item (+1 level, +15% stats)
...
// ─────────────────────────────────────────────────────────────
//  STYLES (top-level — shared by all screens)
// ─────────────────────────────────────────────────────────────
const S = {
    // ── Base
...
// ═══════════════════════════════════════════════════════════════
//  AUDIO — procedural Web Audio SFX (no external files)
// ═══════════════════════════════════════════════════════════════
const AUDIO = {
    ...
    // Element-specific sound for spell types
    playSpell(type) {
    ...
    _scheduleLoop(theme) {
        ...
        const THEMES = {
            ...
            combat: { notes: [146, 155, 164, 174, 164, 155, 146, 138], interval: 0.25, chord: [87, 110, 146], vol: 0.08, type: "square" },
...
// ═══════════════════════════════════════════════════════════════
//  VISUAL EFFECTS — floating text, particle bursts, screen shake
// ═══════════════════════════════════════════════════════════════
function useFX() {
    ...
    const burst = useCallback((color = "#fbbf24", count = 14, opts = {}) => {
        ...
        const cy = opts.y !== undefined ? opts.y : 50;
...
// Overlay renderer — mounted inside combat/game screens
...
function DungeonQuest() {
    ...
    const [activeEnchantment, setActiveEnchantment] = useState(null);
    ...
    // Music manager — changes music based on screen
    useEffect(() => {
    ...
    const startFloor = useCallback((fl, pl, dungeon) => {
        ...
        addMsg(`⬇️ Floor ${fl} — ${bName} (${wName})`, "#a78bfa");
        setScreen("game");
    ...
    const startGame = useCallback(() => {
        ...
        setPlayer(p);
        setQuests(p.quests);
        // Go to dungeon select instead of directly entering floor 1
        setScreen("dungeon_select");
    }, [playerClass, playerName, meta, ngPlus]);

    // Enter a specific named dungeon
    const startDungeon = useCallback((dungeon, pl) => {
        const p = pl || player;
        if (!p) return;
        if (p.level < dungeon.requiredLevel) {
            addMsg(`🔒 Requires Level ${dungeon.requiredLevel}!`, "#ef4444");
            return;
        }
        setCurrentDungeon(dungeon);
        setDungeonFloor(1);
        addMsg(`⚔️ Entering ${dungeon.name}...`, dungeon.colors.accent);
    ...
    const movePlayer = useCallback((dx, dy) => {
        ...
        if (enemy) {
            ...
            setPlayerTurn(true);
            setScreen("combat");
            // Play combat music when engaging with mobs
            if (enemy.isBoss) {
        ...
        if (tile === T.CHEST) {
            ...
            loot.forEach(it => addMsg(`📦 Found: ${it.icon} ${it.name} [${RARITY[it.rarity]?.name}]${it.upgradeLevel > 0 ? ` +${it.upgradeLevel}` : ""}${it.isVariant ? " ~" : ""}`, RARITY[it.rarity]?.color || "#e2e8f0"));
            setPlayer(prev => {
            ...
            // ❤️ BLOCK if enemies remain — must defeat all mobs first!
            if (enemies.length > 0) {
                AUDIO.play("miss");
                fx.shakeScreen(6);
            ...
            // Check if at max dungeon floor — return to hub
            ...
            if (dungeonFloor >= maxFloor) {
                ...
                addMsg(`🌟 ${currentDungeon?.name || "Dungeon"} CLEARED! Returning to hub...`, "#fbbf24");
            ...
            setPlayer(prev => {
                ...
                const healed = {
                    ...prev, floorsCleared: prev.floorsCleared + 1,
                ...
                addMsg(`⬇️ Descending to floor ${newFloor}... (+20% HP/MP)`, "#a78bfa");
            ...
            loot.forEach(it => addMsg(`✨ Secret! ${it.icon} ${it.name} [${RARITY[it.rarity]?.name}]`, RARITY[it.rarity]?.color));
            setPlayer(prev => ({ ...prev, inventory: [...prev.inventory, ...loot] }));
        } else if (tile === T.PORTAL) {
            AUDIO.play("portal");
            fx.burst("#a855f7", 30); fx.flashScreen("#a855f7", 300);
            addMsg("🌀 The portal hums... descend rapidly!", "#a855f7");
            const g = mapData.grid.map(r => [...r]); g[ny][nx] = T.FLOOR;
            setMapData(prev => ({ ...prev, grid: g }));
            setPlayer(prev => {
    ...
    const progressQuests = useCallback((ev) => {
        setQuests(prev => prev.map(q => {
            ...
            if (done) {
                addMsg(`✨ Quest complete: ${p.name}`, "#fde047");
        ...
        setTimeout(() => {
            ...
            setPlayer(prev => {
                ...
                if (prev.divineShield && prev.hp <= dmg) {
                    addCombatMsg("✨ Divine Shield absorbs!", "#f59e0b");
                    AUDIO.play("holy");
                ...
                if (prev.thornsActive && enemy.hp > 0) {
                    const thornsDmg = Math.floor(dmg * prev.thornsDamage);
                    setCombatEnemy(ue => {
                        if (!ue) return ue;
                        const newEnemyHp = Math.max(0, ue.hp - thornsDmg);
                        addCombatMsg(`🟫 THORNS reflect ${thornsDmg}!`, "#92400e");
                        fx.popText(`-${thornsDmg}`, "#92400e", { size: 18, y: 35 });
                        return { ...ue, hp: newEnemyHp };
                    });
                    // Clear thorns after use
                    prev.thornsActive = false;
                }

                if (newHp <= 0) {
                    if (prev.cls === "NECROMANCER" && Math.random() < 0.2) {
                        addCombatMsg("💀 Death Pact — revived!", "#8b5cf6");
                        AUDIO.play("dark");
                        fx.burst("#8b5cf6", 26);
                    ...
                    addCombatMsg(`💀 ${prev.name} falls...`, "#ef4444");
                    AUDIO.play("death");
        ...
        setTimeout(() => {
            ...
            if (action === "attack" || action === "spell") {
                ...
                if (action === "attack") {
                    ...
                    if (res.isAdapted) addCombatMsg("🧠 Enemy adapted! -DMG", "#f87171");

                    // Apply active enchantment effects
                    if (activeEnchantment) {
                        const enchant = ELEMENTAL_ENCHANTMENTS[activeEnchantment];
                        
                        // Blazing: Burn status and prevent adaptation
                        if (enchant.burnOnNextHit) {
                            ue.status = "BURN";
                            ue.statusTurns = 3;
                            addCombatMsg("🔥 Enemy BURNED!", "#ef4444");
                            // Prevent adaptation
                            if (enchant.preventAdaptation) {
                                ue.adaptiveRes = {};
                            }
                        }
                        
                        // Bluster: Explosion effect
                        if (enchant.explosionOnHit) {
                            dmg = Math.floor(dmg * 1.5); // 50% more damage
                            addCombatMsg("⚡ EXPLOSION HIT!", "#3b82f6");
                            fx.burst("#3b82f6", 30);
                        }
                        
                        // Frost: Freeze chance
                        if (enchant.freezeChance && Math.random() < enchant.freezeChance) {
                            ue.status = "FREEZE";
                            ue.statusTurns = 2;
                            addCombatMsg("❄️ Enemy FROZEN!", "#06b6d4");
                        }
                        
                        // Shadow: Guaranteed crit and invis
                        if (enchant.invisNextHit) {
                            isCrit = true;
                            dmg = Math.floor(dmg * 2); // Double damage
                            addCombatMsg("🌑 SHADOW STRIKE!", "#6b7280");
                            fx.burst("#6b7280", 25);
                        }
                        
                        // Earth: Thorns preparation (will be applied when enemy attacks)
                        if (enchant.thorns) {
                            up.thornsActive = true;
                            up.thornsDamage = enchant.thorns;
                            addCombatMsg("🟫 THORNS READY!", "#92400e");
                        }
                    }

                    // Track adaptation (25% per hit) - only if not prevented
                    if (!activeEnchantment || !ELEMENTAL_ENCHANTMENTS[activeEnchantment].preventAdaptation) {
                    ...
                    if (up.shadowFirst && up.cls === "ROGUE") {
                        ...
                        addCombatMsg("⚡ Shadowstrike!", "#a78bfa");
                        AUDIO.play("dark");
                        fx.burst("#a78bfa", 20);
                    }
                    ...
                    if (up.mana < manaCost) { addCombatMsg("❌ Not enough mana!", "#ef4444"); AUDIO.play("miss"); setPlayerTurn(true); setAnimating(false); return; }
                    up.mana = Math.max(0, up.mana - manaCost);

                    // Procedural spell audio by element
                    ...
                    if (sp.dmg[0] < 0) {
                        ...
                        addCombatMsg(`💚 +${healAmt} HP`, "#4ade80");
                        AUDIO.play("heal");
                        fx.burst("#4ade80", 18);
                        ...
                        addCombatMsg(`✨ ${spellName} activated!`, "#a78bfa");
                        fx.burst("#a78bfa", 20);
                        ...
                        if (sp.status && Math.random() < 0.45) {
                            ...
                            addCombatMsg(`✨ ${ue.name} is ${sp.status}!`, "#a78bfa");
                        }
                    }
                ...
                if (ue.status === "MARK" && action === "attack") { dmg = ue.hp; addCombatMsg(`💀 DEATH MARK!`, "#8b5cf6"); AUDIO.play("dark"); fx.flashScreen("#8b5cf6", 250); fx.burst("#8b5cf6", 32); }

                if (dmg > 0) {
                    ue.hp = Math.max(0, ue.hp - dmg);
                    addCombatMsg(`${isCrit ? "💥 CRIT! " : ""}${dmg} dmg → ${ue.name}`, isCrit ? "#fbbf24" : "#e2e8f0");
                    if (isCrit) {
                ...
                if (ue.status && STATUS_EFFECTS[ue.status]?.kind === "dot") {
                    ...
                    addCombatMsg(`🩸 ${dot} ${ue.status} dmg`, "#fb923c");
                }
                ...
                if (up.cls === "NECROMANCER" && ue.hp <= 0) {
                    ...
                    addCombatMsg(`👻 Soul Harvest: +30 MP`, "#8b5cf6");
                }

                setAnimating(false);

                if (ue.hp <= 0) {
                    addCombatMsg(`⚔️ ${ue.name} defeated!`, "#4ade80");
                    setEnemies(prev => prev.filter(e => e.uid !== combatEnemy.uid));
                    
                    // Check for elemental enchantment activation
                    if (!activeEnchantment && ue.element) {
                        const enchantmentKeys = Object.keys(ELEMENTAL_ENCHANTMENTS);
                        const matchingEnchantment = enchantmentKeys.find(key => key.includes(ue.element));
                        if (matchingEnchantment && Math.random() < 0.3) { // 30% chance to get enchantment
                            const enchantment = ELEMENTAL_ENCHANTMENTS[matchingEnchantment];
                            setActiveEnchantment(matchingEnchantment);
                            addMsg(`✨ ${enchantment.icon} ${enchantment.name} ENCHANTMENT ACTIVATED!`, enchantment.color);
                            addMsg(`${enchantment.desc}`, enchantment.color);
                            fx.burst(enchantment.color, 40);
                            fx.flashScreen(enchantment.color, 300);
                        }
                    }
                    
                    // Apply active enchantment bonuses
                    let expBonus = 1.0;
                    let goldBonus = 1.0;
                    let extraConsumables = 0;
                    
                    if (activeEnchantment) {
                        const enchant = ELEMENTAL_ENCHANTMENTS[activeEnchantment];
                        expBonus = enchant.expMult;
                        goldBonus = enchant.goldMult;
                        extraConsumables = enchant.consumableBonus || 0;
                        
                        // Handle duration-based enchantments
                        if (enchant.duration === "next_kill") {
                            addMsg(`⚡ ${enchant.name} enchantment consumed!`, enchant.color);
                            setActiveEnchantment(null);
                        }
                    }
                    
                    // Adaptive Resistance & Chaos Engine Scaling
                    ...
                    const finalExp = Math.floor(ue.exp * decayMult * expBonus);
                    const finalGold = Math.floor(ue.gold * decayMult * goldBonus);

                    addMsg(`✅ Defeated ${ue.icon} ${ue.name}: +${finalExp} EXP, +${finalGold} gold ${decayMult < 1 ? `(Decay: ${Math.floor(decayMult * 100)}%)` : ""} ${expBonus > 1 ? `(Enchanted!)` : ""}`, "#4ade80");
                    ...
                    if (up.level > prevLevel) {
                        addMsg(`⬆️ LEVEL UP! Now Lv.${up.level}!`, "#fbbf24");
                        AUDIO.play("levelup");
                    ...
                    if (drops > 0) {
                        ...
                        loot.forEach(d => addMsg(`🎁 Loot: ${d.icon} ${d.name} [${RARITY[d.rarity]?.name}]${d.upgradeLevel > 0 ? ` +${d.upgradeLevel}` : ""}${d.isVariant ? " ~" : ""}`, RARITY[d.rarity]?.color));
                        up.inventory = [...up.inventory, ...loot];
                        loot.forEach(d => setMeta(m => recordItemSeen(m, d.id)));
                        // Boss drops trigger special fx
                        if (ue.isBoss) {
                            fx.popText("EPIC LOOT!", "#a855f7", { size: 28, y: 30 });
                            fx.burst("#a855f7", 30);
                        }
                    }
                    // Faction rep (biome-based)
    ...
    const useItemInCombat = useCallback((item) => {
        ...
        if (item.heal) {
            ...
            addCombatMsg(`🧪 +${item.heal} HP`, "#4ade80");
            AUDIO.play("heal");
            fx.burst("#4ade80", 16);
        ...
        if (item.mana) {
            ...
            addCombatMsg(`💧 +${item.mana} MP`, "#60a5fa");
            AUDIO.play("spell");
            fx.burst("#60a5fa", 14);
        ...
        if (item.slot === "scroll" && item.spell && !up.spells.includes(item.spell)) {
            ...
            addCombatMsg(`📜 Learned ${item.spell}!`, "#a78bfa");
            AUDIO.play("levelup");
            fx.burst("#a78bfa", 26);
    ...
    const equipItem = useCallback((item) => {
        ...
        if (!check.can) {
            if (check.reason === "weapon") {
                addMsg(`❌ ${CLASSES[player.cls]?.name}s cannot equip this weapon type!`, "#ef4444");
            } else {
                addMsg(`❌ ${CLASSES[player.cls]?.name}s cannot wear ${check.type} armor!`, "#ef4444");
            }
            fx.shakeScreen(4);
            AUDIO.play("miss");
            return;
        ...
        setPlayer(prev => {
            ...
            if (old) inv.push(old);
            addMsg(`✅ Equipped ${item.icon} ${item.name}`, RARITY[item.rarity]?.color);
    ...
    const handleWeaponUpgrade = useCallback((item) => {
        ...
        if ((item.upgradeLevel || 0) >= maxLvl) {
            addMsg(`❌ Weapon is already at max level (+${maxLvl})`, "#ef4444");
            return;
        }
        if (player.gold < cost) {
            addMsg(`❌ Need ${cost}g to upgrade!`, "#ef4444");
            return;
        }
        ...
        addMsg(`🔨 Upgraded ${item.name} to +${(item.upgradeLevel || 0) + 1}!`, "#f59e0b");
    }, [player, addMsg, fx]);
    ...
    const useItem = useCallback((item) => {
        ...
        if (item.isPermanent && item.spell) {
            if (player.spells.includes(item.spell)) {
                addMsg(`❌ You already know ${item.spell}!`, "#ef4444");
                return;
            }
            setPlayer(prev => ({
                ...prev,
            ...
            addMsg(`✨ UNLOCKED PERMANENT SPELL: ${item.spell}!`, "#ef4444");
            AUDIO.play("holy");
            fx.flashScreen("#ef4444", 600);
            fx.burst("#ef4444", 60);
    ...
    const craftItem = useCallback((recipe) => {
        ...
        for (const [m, n] of Object.entries(needed)) {
            if ((have[m] || 0) < n) { addMsg(`❌ Need ${n} × ${m}`, "#ef4444"); return; }
        }
        if (player.gold < (recipe.gold || 0)) { addMsg("❌ Not enough gold", "#ef4444"); return; }
        // Consume
        for (const [m, n] of Object.entries(needed)) {
            let toRemove = n;
        ...
        if (produced) {
            ...
            addMsg(`🔨 Crafted ${produced.icon} ${produced.name}!`, RARITY[produced.rarity]?.color);
        }
    ...
    const pickSubclass = useCallback((sub) => {
        ...
        addMsg(`🌟 Subclass chosen: ${CLASSES[player.cls]?.subclasses?.[sub]?.name}`, "#a78bfa");
    }, [player, addMsg]);
    ...
    useEffect(() => {
        ...
        if (newly.length > 0) {
            newly.forEach(a => addMsg(`🏆 Achievement: ${a.name}!`, "#fde047"));
    ...
    const renderHud = () => player && (
        <div style={{ flex: "0 0 auto", padding: "6px 12px", background: "#0d0d1a", borderBottom: "1px solid #1e1e2e", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
            ...
            <div style={{ display: "flex", gap: 10, fontSize: 11, alignItems: "center", flexWrap: "wrap" }}>
                <span style={{ color: "#a78bfa" }}>🏰 F{floor}</span>
                <span style={{ color: "#fbbf24" }}>💰{player.gold}g</span>
                <span style={{ color: "#9ca3af" }}>⚔{stats?.totalAtk || 0} 🛡{stats?.totalDef || 0}</span>
            ...
            <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                <button style={S.hudBtn} onClick={() => { AUDIO.play("click"); setScreen("inventory"); }}>🎒 I</button>
                <button style={S.hudBtn} onClick={() => { AUDIO.play("click"); setScreen("menu"); }}>☰ Menu</button>
                <button style={{ ...S.hudBtn, color: muted ? "#6b7280" : "#60a5fa" }} onClick={() => { const m = !muted; setMuted(m); AUDIO.muted = m; if (!m) AUDIO.play("click"); }} title={muted ? "Unmute" : "Mute"}>{muted ? "🔇" : "🔊"}</button>
            </div>
        </div>
    );
        ...
        <div style={S.root}>
            <div style={{ ...S.screen, background: "radial-gradient(ellipse at center, #1a0a2e 0%, #0d0d0d 70%)" }}>
                ...
                <div style={S.titleContent}>
                    <div style={{ fontSize: 80, marginBottom: 8, filter: "drop-shadow(0 0 30px #a855f7)" }}>⚔</div>
                    ...
                    <div>
                        <button style={S.bigBtn} onClick={() => setScreen("charselect")}>⚔️ NEW GAME</button>
                        <button style={{ ...S.bigBtn, background: "linear-gradient(135deg, #374151, #1f2937)" }} onClick={() => setScreen("loadgame")}>📂 LOAD GAME</button>
                    </div>
                    <div style={{ marginTop: 16, display: "flex", gap: 8 }}>
                        <button style={S.smallBtn} onClick={() => setScreen("leaderboards")}>🏆 Leaderboards</button>
                        <button style={S.smallBtn} onClick={() => setScreen("bestiary")}>📖 Bestiary</button>
                        <button style={S.smallBtn} onClick={() => setScreen("collectionlog")}>📚 Collection</button>
                    </div>
    ...
    if (screen === "charselect") return (
        <div style={S.root}>
            <div style={{ ...S.screen, background: "#0d0d15", overflowY: "auto", padding: "20px 10px" }}>
                <h2 style={{ textAlign: "center", color: "#e2e8f0", fontFamily: "serif", fontSize: 28, marginBottom: 4 }}>⚔ Choose Your Class</h2>
                        ...
                        return (
                            <div key={key} style={{
                                ...
                                {runs > 0 && <div style={{ fontSize: 10, color: "#22c55e", marginBottom: 4 }}>
                                    🏆 {runs} runs · +{mb.atk}A +{mb.def}D +{mb.hp}HP
                                </div>}
                                ...
                                <div key={d.id} className="doe-card-in" style={{ ...S.dungeonCard(d.colors.accent), animationDelay: `${i * 0.05}s`, opacity: lockedDungeon ? 0.6 : 1, filter: lockedDungeon ? "grayscale(80%)" : "none" }}>
                                    ...
                                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                                        ...
                                        <div style={{ background: lockedDungeon ? "#ef444433" : "#22c55e33", color: lockedDungeon ? "#ef4444" : "#22c55e", padding: "3px 8px", borderRadius: 4, fontSize: 10, fontWeight: "bold", border: `1px solid ${lockedDungeon ? '#ef444466' : '#22c55e66'}` }}>
                                            {lockedDungeon ? `🔒 LVL ${d.requiredLevel}` : `Tier ${d.tier}`}
                                        </div>
                                    </div>
                                        ...
                                        {!lockedDungeon && (
                                            <div style={{ display: "flex", flexDirection: "column", gap: 8, alignItems: "flex-end" }}>
                                                <label style={{ display: "flex", alignItems: "center", gap: 6, cursor: "pointer" }}>
                                                    ...
                                                    />
                                                    <span style={{ color: hardcore ? "#ef4444" : "#9ca3af", fontSize: 11, fontWeight: hardcore ? "bold" : "normal" }}>
                                                        💀 HARDCORE MODE
                                                    </span>
                                                </label>
                                                <button style={{
                                                    background: hardcore ? "linear-gradient(135deg, #ef4444, #dc2626)" : "linear-gradient(135deg, #22c55e, #16a34a)", 
                                                    color: "white",
                                                    border: "none", padding: "6px 16px", borderRadius: 4, fontSize: 12, fontWeight: "bold",
                                                    cursor: "pointer", 
                                                    boxShadow: hardcore ? "0 2px 10px rgba(239, 68, 68, 0.3)" : "0 2px 10px rgba(34, 197, 94, 0.3)"
                                                }} onClick={() => startDungeon(d, selectedDifficulty)}>
                                                    {hardcore ? "ENTER HARDCORE ⚠" : "ENTER DUNGEON ➡"}
                                                </button>
                                            </div>
                                        )}
                                    </div>
    ...
    if (screen === "loadgame") return (
        <div style={S.root}>
            <div style={{ ...S.screen, background: "#0d0d15", padding: 20, overflowY: "auto" }}>
                <h2 style={{ color: "#e2e8f0", fontFamily: "serif", fontSize: 22, marginBottom: 8 }}>📂 Load Game</h2>
            ...
            <div style={S.root}>
                <div className={fx.shake ? "doe-shake" : ""} style={{ ...S.screen, background: "radial-gradient(ellipse at center, #1a0a0a 0%, #0d0505 70%)", display: "flex", flexDirection: "column", position: "relative" }}>
                    ...
                    <div style={{ flex: "0 0 auto", padding: "14px 20px", borderBottom: "1px solid #2d1a1a" }}>
                        ...
                        {activeEnchantment && (
                            <div style={{ background: ELEMENTAL_ENCHANTMENTS[activeEnchantment].color + "22", border: `1px solid ${ELEMENTAL_ENCHANTMENTS[activeEnchantment].color}`, borderRadius: 6, padding: 8, marginTop: 8, textAlign: "center" }}>
                                <div style={{ color: ELEMENTAL_ENCHANTMENTS[activeEnchantment].color, fontWeight: "bold", fontSize: 12, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
                                    <span>{ELEMENTAL_ENCHANTMENTS[activeEnchantment].icon}</span>
                                    <span>{ELEMENTAL_ENCHANTMENTS[activeEnchantment].name} ENCHANTMENT</span>
                                </div>
                                <div style={{ color: ELEMENTAL_ENCHANTMENTS[activeEnchantment].color, fontSize: 10, marginTop: 2 }}>
                                    {ELEMENTAL_ENCHANTMENTS[activeEnchantment].desc}
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    <div style={{ flex: "0 0 auto", borderTop: "1px solid #2d1a1a", padding: "8px 16px" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                            ...
                            <span style={{ color: "#fbbf24", fontSize: 12 }}>💰{player.gold}g</span>
                        </div>
                    ...
                    <div style={{ flex: "0 0 auto", borderTop: "1px solid #2d1a1a", padding: "8px 12px", background: "#0d0a0a" }}>
                        <div style={{ display: "flex", gap: 6, marginBottom: 6, flexWrap: "wrap" }}>
                            <button style={{ ...S.combatBtn("#ef4444"), opacity: playerTurn && !animating ? 1 : 0.4 }} disabled={!playerTurn || animating} onClick={() => { AUDIO.play("click"); doPlayerAction("attack"); }}>⚔️ Attack</button>
                            <button style={{ ...S.combatBtn("#22c55e"), opacity: playerTurn && !animating ? 1 : 0.4 }} disabled={!playerTurn || animating} onClick={() => { AUDIO.play("click"); doPlayerAction("flee"); }}>💨 Flee</button>
                            <button style={{ ...S.combatBtn(muted ? "#6b7280" : "#60a5fa"), marginLeft: "auto" }} onClick={() => { const m = !muted; setMuted(m); AUDIO.muted = m; if (!m) AUDIO.play("click"); }} title={muted ? "Unmute" : "Mute"}>{muted ? "🔇" : "🔊"}</button>
                        </div>
            ...
            <div style={S.root}>
                <div style={{ ...S.screen, background: "#0d0d15", display: "flex", flexDirection: "column" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px", borderBottom: "1px solid #2d2d3d" }}>
                        <h2 style={{ color: "#e2e8f0", fontFamily: "serif", fontSize: 22 }}>🎒 Inventory & Equipment</h2>
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(150px,1fr))", gap: 6 }}>
                                {player.inventory.filter(it => {
                                    ...
                                    <div key={item.uid} style={{ ...S.invItem, borderColor: (RARITY[item.rarity]?.color || "#2d2d3d") + "55", background: RARITY[item.rarity]?.glow || "#1a1a2e", opacity: canClassEquip(player.cls, item).can ? 1 : 0.6 }}>
                                        ...
                                        {!canClassEquip(player.cls, item).can && <div style={{ fontSize: 8, color: "#ef4444", marginBottom: 4, fontWeight: "bold", textAlign: "center" }}>🚫 {CLASSES[player.cls]?.name.toUpperCase()} RESTRICTED</div>}
                                        ...
                                        <div style={{ display: "flex", flexWrap: "wrap", gap: 3, justifyContent: "center", marginBottom: 6 }}>
                                            ...
                                            {item.spell && <span style={S.statBadge("#a78bfa")}>📜 {item.spell}</span>}
                                            {item.upgradeLevel > 0 && <span style={S.statBadge("#f59e0b")}>+{item.upgradeLevel} 🔨</span>}
                                        </div>
    ...
    if (screen === "shop" && player) return (
        <div style={S.root}>
            <div style={{ ...S.screen, background: "#0d0d15", display: "flex", flexDirection: "column" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px", borderBottom: "1px solid #2d2d3d" }}>
                    <h2 style={{ color: "#fbbf24", fontFamily: "serif", fontSize: 22 }}>🏪 Dungeon Merchant</h2>
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        <span style={{ color: "#fbbf24", fontSize: 14 }}>💰 {player.gold}g</span>
            ...
            <div style={{ ...S.screen, background: "#0d0d15", overflowY: "auto", padding: 20 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: 10 }}>
                    {[
                        ...
                        ["quests", "📜", "Quests", "(Q)"],
                        ["skilltree", "🌳", "Skill Tree", "(K)"],
                        ["crafting", "🔨", "Crafting", "(C)"],
                        ["factions", "🏴", "Factions", "(F)"],
                        ["bestiary", "📖", "Bestiary", "(B)"],
                        ["achievements", "🏆", "Achievements", "(T)"],
                        ["housing", "🏠", "Hero's Rest", "(H)"],
                        ["subclass", "🌟", "Subclass", player.level >= SUBCLASS_UNLOCK_LEVEL ? "" : "(Lv20)"],
        ...
        <div style={S.root}>
            <div style={{ ...S.screen, background: "#0d0d15", padding: 20, overflowY: "auto" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                    <h2 style={{ color: "#e2e8f0", fontFamily: "serif", fontSize: 22 }}>📜 Quest Log</h2>
                    <div style={{ display: "flex", gap: 8 }}>
                        <button style={S.smallBtn} onClick={() => { const q = rollQuests(floor, 3); setQuests(q); addMsg("📜 New quests!", "#a78bfa"); }}>🔄 Roll Quests</button>
                        <button style={S.closeBtn} onClick={() => setScreen("game")}>✕ Close</button>
                    </div>
                </div>
                    ...
                    {quests.map(q => {
                        ...
                        return (
                            <div key={q.uid} style={{ background: "#1a1a2e", padding: 12, borderRadius: 6, border: q.complete ? "1px solid #22c55e" : "1px solid #2d2d3d" }}>
                                <div style={{ color: q.complete ? "#22c55e" : "#fde047", fontWeight: "bold", fontSize: 14 }}>{fmt.name} {q.complete && "✓"}</div>
    ...
    if (screen === "skilltree" && player) {
        ...
        return (
            <div style={S.root}>
                <div style={{ ...S.screen, background: "#0d0d15", padding: 20, overflowY: "auto" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                        <h2 style={{ color: "#e2e8f0", fontFamily: "serif", fontSize: 22 }}>🌳 {CLASSES[player.cls]?.name} Skill Tree</h2>
                        <div style={{ display: "flex", gap: 8 }}>
                                ...
                                {byTier[t].map(n => {
                                    ...
                                    return (
                                        <div key={n.id} style={{ background: unlocked ? "#1a2e1a" : "#1a1a2e", padding: 10, borderRadius: 5, border: unlocked ? "1px solid #22c55e" : "1px solid #2d2d3d", cursor: unlocked ? "default" : "pointer" }}
                                            ...
                                            <div style={{ color: unlocked ? "#22c55e" : "#e2e8f0", fontWeight: "bold", fontSize: 12 }}>{n.name} {unlocked && "✓"}</div>
    ...
    if (screen === "crafting" && player) {
        ...
        return (
            <div style={S.root}>
                <div style={{ ...S.screen, background: "#0d0d15", padding: 20, overflowY: "auto" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                        <h2 style={{ color: "#f59e0b", fontFamily: "serif", fontSize: 22 }}>🔨 Crafting Forge</h2>
                        <div style={{ display: "flex", gap: 8 }}>
                            <span style={{ color: "#fbbf24" }}>💰 {player.gold}g</span>
                            <button style={S.closeBtn} onClick={() => setScreen("game")}>✕ Close</button>
                        </div>
                            ...
                            return (
                                <div key={r.id} style={{ background: "#1a1a2e", padding: 12, borderRadius: 6, border: canCraft ? "1px solid #22c55e" : "1px solid #2d2d3d" }}>
                                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                                        <span style={{ fontSize: 26 }}>{produced?.icon || "📦"}</span>
                                        <div>
                                    ...
                                    <button style={{ ...S.smBtn("#f59e0b"), opacity: canCraft ? 1 : 0.4 }} disabled={!canCraft} onClick={() => craftItem(r)}>🔨 Craft</button>
                                </div>
                            );
                        })}
                    </div>
    ...
    if (screen === "factions") return (
        <div style={S.root}>
            <div style={{ ...S.screen, background: "#0d0d15", padding: 20, overflowY: "auto" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                    <h2 style={{ color: "#e2e8f0", fontFamily: "serif", fontSize: 22 }}>🏴 Factions & Reputation</h2>
                    <button style={S.closeBtn} onClick={() => setScreen("game")}>✕ Close</button>
                </div>
    ...
    if (screen === "bestiary") return (
        <div style={S.root}>
            <div style={{ ...S.screen, background: "#0d0d15", padding: 20, overflowY: "auto" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                    <h2 style={{ color: "#e2e8f0", fontFamily: "serif", fontSize: 22 }}>📖 Bestiary</h2>
    ...
    if (screen === "achievements") return (
        <div style={S.root}>
            <div style={{ ...S.screen, background: "#0d0d15", padding: 20, overflowY: "auto" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                    <h2 style={{ color: "#fde047", fontFamily: "serif", fontSize: 22 }}>🏆 Achievements</h2>
                    <button style={S.closeBtn} onClick={() => setScreen("game")}>✕ Close</button>
                </div>
                    ...
                    {ACHIEVEMENTS.map(a => {
                        ...
                        return (
                            <div key={a.id} style={{ background: "#1a1a2e", padding: 10, borderRadius: 5, border: p.done ? "1px solid #fde047" : "1px solid #2d2d3d", opacity: p.done ? 1 : 0.8 }}>
                                <div style={{ color: p.done ? "#fde047" : "#e2e8f0", fontWeight: "bold" }}>{p.done ? "🏆 " : ""}{a.name}</div>
    ...
    if (screen === "housing" && player) return (
        <div style={S.root}>
            <div style={{ ...S.screen, background: "#0d0d15", padding: 20, overflowY: "auto" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                    <h2 style={{ color: "#10b981", fontFamily: "serif", fontSize: 22 }}>🏠 Hero's Rest</h2>
                    <div style={{ display: "flex", gap: 8 }}>
                        <span style={{ color: "#fbbf24" }}>💰 {player.gold}g</span>
                    ...
                    {HOUSING_UPGRADES.map(u => {
                        ...
                        return (
                            <div key={u.id} style={{ background: "#1a1a2e", padding: 12, borderRadius: 6, border: owned ? "1px solid #22c55e" : affordable ? "1px solid #a78bfa" : "1px solid #2d2d3d" }}>
                                <div style={{ color: owned ? "#22c55e" : "#e2e8f0", fontWeight: "bold" }}>{u.name} {owned && "✓"}</div>
    ...
    if (screen === "subclass" && player) {
        ...
        return (
            <div style={S.root}>
                <div style={{ ...S.screen, background: "#0d0d15", padding: 20, overflowY: "auto" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                        <h2 style={{ color: "#e2e8f0", fontFamily: "serif", fontSize: 22 }}>🌟 Choose Subclass</h2>
                        <button style={S.closeBtn} onClick={() => setScreen("game")}>✕ Close</button>
                    </div>
                    {player.subclass ? (
    ...
    // ─────────────────────────────────────────────────────────────
    //  SCREEN — COLLECTION LOG
    // ─────────────────────────────────────────────────────────────
    if (screen === "collectionlog") return (
        <div style={S.root}>
            <div style={{ ...S.screen, background: "#0d0d15", padding: 20, overflowY: "auto" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                    <h2 style={{ color: "#e2e8f0", fontFamily: "serif", fontSize: 22 }}>📚 Collection Log</h2>
                    <button style={S.closeBtn} onClick={() => (player ? setScreen("game") : setScreen("title"))}>✕ Close</button>
                </div>
                <p style={{ color: "#6b7280", fontSize: 12, marginBottom: 16 }}>
                    ...
                    {ALL_ITEMS.map(it => {
                        ...
                        return (
                            <div key={it.id} style={{ background: "#1a1a2e", padding: 8, borderRadius: 5, border: "1px solid #2d2d3d", opacity: found ? 1 : 0.3 }}>
                                ...
                                {found && <div style={{ fontSize: 9, color: "#6b7280", textAlign: "center" }}>×{found.count}</div>}
                            </div>
                        );
                    })}
                </div>
    ...
    //  SCREEN — TITLES
    // ─────────────────────────────────────────────────────────────
    if (screen === "titles") return (
        <div style={S.root}>
            <div style={{ ...S.screen, background: "#0d0d15", padding: 20, overflowY: "auto" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                    <h2 style={{ color: "#fde047", fontFamily: "serif", fontSize: 22 }}>👑 Titles</h2>
                    <button style={S.closeBtn} onClick={() => setScreen("game")}>✕ Close</button>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 8 }}>
                    {Object.entries(TITLES).map(([key, t]) => {
                        const unlocked = meta.unlockedTitles?.includes(key) || key === "NOVICE";
                        return (
                            <div key={key} style={{ background: "#1a1a2e", padding: 10, borderRadius: 5, border: activeTitle === key ? `1px solid ${t.color}` : "1px solid #2d2d3d", opacity: unlocked ? 1 : 0.4, cursor: unlocked ? "pointer" : "default" }}
                                onClick={() => unlocked && setActiveTitle(key)}>
                                <div style={{ color: t.color, fontWeight: "bold" }}>{t.name}</div>
                                <div style={{ color: "#9ca3af", fontSize: 10 }}>{t.desc}</div>
                            </div>
                        );
                    })}
                </div>
    ...
    //  SCREEN — LEADERBOARDS
    // ─────────────────────────────────────────────────────────────
    if (screen === "leaderboards") return (
        <div style={S.root}>
            <div style={{ ...S.screen, background: "#0d0d15", padding: 20, overflowY: "auto" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                    <h2 style={{ color: "#fbbf24", fontFamily: "serif", fontSize: 22 }}>🏆 Leaderboards</h2>
                    <button style={S.closeBtn} onClick={() => (player ? setScreen("game") : setScreen("title"))}>✕ Close</button>
                </div>
                <div style={{ background: "#1a1a2e", padding: 12, borderRadius: 6 }}>
                    {(meta.leaderboard || []).length === 0 && <p style={{ color: "#6b7280" }}>No runs yet.</p>}
                    {(meta.leaderboard || []).map((e, i) => (
                        <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: 4, borderBottom: "1px solid #2d2d3d", fontSize: 12 }}>
                            <span style={{ color: "#fbbf24" }}>#{i + 1}</span>
                            <span style={{ color: "#e2e8f0" }}>{e.name}</span>
                            <span style={{ color: CLASSES[e.cls]?.color }}>{CLASSES[e.cls]?.icon} {CLASSES[e.cls]?.name}</span>
                            <span style={{ color: "#a78bfa" }}>Floor {e.floor}</span>
                            <span style={{ color: "#9ca3af" }}>{e.kills} kills</span>
                            <span style={{ color: e.died ? "#ef4444" : "#22c55e" }}>{e.died ? "💀 Died" : "✓ Survived"}</span>
                        </div>
                    ))}
                </div>
                <h3 style={{ ...S.sectionH, marginTop: 20 }}>Recent Ghosts</h3>
                <div style={{ background: "#1a1a2e", padding: 12, borderRadius: 6 }}>
                    {(meta.ghosts || []).length === 0 && <p style={{ color: "#6b7280" }}>No ghosts yet.</p>}
                    {(meta.ghosts || []).map((g, i) => (
                        <div key={i} style={{ color: "#9ca3af", fontSize: 11, padding: 3 }}>
                            💀 {g.name} ({CLASSES[g.cls]?.name}) fell on F{g.floor} to {g.cause}
                        </div>
                    ))}
                </div>
            </div>
    ...
    //  SCREEN — SAVE GAME
    // ─────────────────────────────────────────────────────────────
    if (screen === "savegame" && player) return (
        <div style={S.root}>
            <div style={{ ...S.screen, background: "#0d0d15", padding: 20, overflowY: "auto" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                    <h2 style={{ color: "#e2e8f0", fontFamily: "serif", fontSize: 22 }}>💾 Save Game</h2>
                    <button style={S.closeBtn} onClick={() => setScreen("game")}>✕ Close</button>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 10 }}>
                    {listCharacters().map(({ slot, save }) => (
                        <div key={slot} style={{ background: "#1a1a2e", padding: 12, borderRadius: 6, border: "1px solid #2d2d3d" }}>
                            <div style={{ color: "#a78bfa", fontSize: 12, marginBottom: 4 }}>Slot {slot}</div>
                            {save && <div style={{ color: "#9ca3af", fontSize: 11, marginBottom: 8 }}>{save.player?.name} Lv.{save.player?.level} F{save.floor}</div>}
                            <button style={S.smallBtn} onClick={() => {
                                saveCharacter(slot, { player, floor, mapData, enemies, quests });
                                addMsg(`💾 Saved to slot ${slot}`, "#22c55e");
                                setScreen("game");
                            }}>Save Here</button>
                        </div>
                    ))}
    ...
    //  SCREEN — TOWN
    // ─────────────────────────────────────────────────────────────
    if (screen === "town" && town) return (
        <div style={S.root}>
            <div style={{ ...S.screen, background: "#0d0d15", padding: 20, overflowY: "auto" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                    <h2 style={{ color: "#e2e8f0", fontFamily: "serif", fontSize: 22 }}>🏘️ {town.name}</h2>
                    <button style={S.closeBtn} onClick={() => setScreen("game")}>✕ Leave</button>
                </div>
                <p style={{ color: "#9ca3af", marginBottom: 16 }}>{town.flavor}</p>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 10 }}>
                    {town.npcs.map(npc => (
                        <div key={npc.uid} style={{ background: "#1a1a2e", padding: 12, borderRadius: 6, border: "1px solid #2d2d3d" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                                <span style={{ fontSize: 28 }}>{npc.icon}</span>
                                <div>
                                    <div style={{ color: FACTIONS[npc.faction]?.color || "#e2e8f0", fontWeight: "bold" }}>{npc.name}</div>
                                    <div style={{ color: "#6b7280", fontSize: 10 }}>{npc.role}</div>
                                </div>
                            </div>
                            <div style={{ color: "#9ca3af", fontSize: 11, fontStyle: "italic", marginBottom: 6 }}>"{npc.dialogue[Math.floor(Math.random() * npc.dialogue.length)]}"</div>
                            <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                                {npc.services.map(sv => (
                                    <button key={sv} style={S.smBtn("#60a5fa")} onClick={() => {
                                        if (sv === "heal") {
                                            const cost = 50;
                                            if (player.gold < cost) { addMsg("Not enough gold", "#ef4444"); return; }
                                            setPlayer(p => ({ ...p, gold: p.gold - cost, hp: stats?.maxHp || p.hp }));
                                            addMsg("Healed!", "#22c55e");
                                        } else if (sv === "quest") {
                                            const nq = rollQuests(floor, 1)[0];
                                            if (nq) { setQuests(q => [...q, nq]); addMsg(`📜 New quest: ${nq.name}`, "#fde047"); }
                                        } else if (sv === "rest") {
                                            setPlayer(p => ({ ...p, hp: stats?.maxHp || p.hp, mana: stats?.maxMana || p.mana }));
                                            addMsg("Rested. Full HP/MP.", "#22c55e");
                                        } else if (sv === "craft" || sv === "socket") {
                                            setScreen("crafting");
                                        } else {
                                            addMsg(`${npc.name} offers ${sv} (not yet implemented)`, "#9ca3af");
                                        }
                                    }}>{sv}</button>
                                ))}
                            </div>
...
//  SUBCOMPONENTS
// ═══════════════════════════════════════════════════════════════
function HpBar({ value, max, big }) {
    const pct = Math.max(0, value / max * 100);
    return (
        <div style={{ display: "flex", alignItems: "center", gap: 4, flex: big ? 1 : "none" }}>
            <span style={{ fontSize: 9, color: "#9ca3af" }}>❤</span>
            <div style={{ width: big ? "100%" : 80, height: big ? 8 : 5, background: "#2d1a1a", borderRadius: 3 }}>
                <div style={{ height: "100%", borderRadius: 3, background: pct > 50 ? "#22c55e" : pct > 25 ? "#f59e0b" : "#ef4444", width: `${pct}%`, transition: "width 0.3s" }} />
            </div>
            <span style={{ fontSize: 9, color: "#9ca3af" }}>{value}/{max}</span>
        </div>
    );
}

function MpBar({ value, max, big }) {
    const pct = Math.max(0, value / max * 100);
    return (
        <div style={{ display: "flex", alignItems: "center", gap: 4, flex: big ? 1 : "none" }}>
            <span style={{ fontSize: 9, color: "#9ca3af" }}>💎</span>
            <div style={{ width: big ? "100%" : 60, height: big ? 8 : 5, background: "#1a1a3d", borderRadius: 3 }}>
                <div style={{ height: "100%", borderRadius: 3, background: "#3b82f6", width: `${pct}%`, transition: "width 0.3s" }} />
            </div>
            <span style={{ fontSize: 9, color: "#9ca3af" }}>{value}/{max}</span>
        </div>
    );
...
function MapView({ mapData, enemies, player, movePlayer }) {
    const { grid, playerPos, visited } = mapData;
    const tileColors = {
        [T.FLOOR]: BIOMES[mapData.biome]?.floor || "#1a1a28",
        [T.WALL]: BIOMES[mapData.biome]?.wall || "#0a0a14",
        [T.DOOR]: "#3b2a0a",
        [T.CHEST]: "#2a1a0a",
        [T.STAIRS]: "#0a1a2a",
        [T.SHRINE]: "#1a0a2a",
    ...
    const tileIcons = {
        [T.CHEST]: "📦", [T.STAIRS]: "⬇", [T.SHRINE]: "⛪", [T.TRAP]: "⚠",
        [T.SHOP]: "🏪", [T.TOWN]: "🏘", [T.PUZZLE]: "🧩", [T.PORTAL]: "🌀",
        [T.FOUNTAIN]: "⛲", [T.SECRET]: "✨", [T.FORGE]: "🔨",
    };
    return (
        <div style={{ flex: "0 0 auto", position: "relative", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center", padding: 4 }}>
            <div style={{ position: "relative" }}>
                {grid.map((row, ry) => (
                    <div key={ry} style={{ display: "flex" }}>
                        {row.map((cell, rx) => {
                            const isPlayer = playerPos.x === rx && playerPos.y === ry;
                            const enemy = enemies.find(e => e.x === rx && e.y === ry);
                            const seen = visited?.[ry]?.[rx];
                            const isFloor = cell !== T.WALL;
                            const opacity = seen ? 1 : 0.15;
                            return (
                                <div key={rx} style={{
                                    width: TILE, height: TILE,
                                    background: tileColors[cell] || "#0a0a14",
                                    border: cell === T.WALL ? "1px solid #0d0d18" : "1px solid #14141e",
                                    display: "flex", alignItems: "center", justifyContent: "center",
                                    fontSize: 17, cursor: isFloor ? "pointer" : "default",
                                    position: "relative", opacity,
                                    boxShadow: cell === T.SHRINE ? "inset 0 0 10px #a855f733" : cell === T.SHOP ? "inset 0 0 8px #10b98133" : "none",
                                }} onClick={() => {
                                    if (!isFloor || isPlayer) return;
                                    const dx = rx - playerPos.x, dy = ry - playerPos.y;
                                    if (Math.abs(dx) <= 1 && Math.abs(dy) <= 1) movePlayer(dx, dy);
                                }}>
                                    {tileIcons[cell] && !enemy && !isPlayer && <span style={{ fontSize: 15 }}>{tileIcons[cell]}</span>}
                                    {enemy && !isPlayer && seen && (
                                        <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                                            <span style={{ fontSize: 17, filter: enemy.isBoss ? "drop-shadow(0 0 4px #fbbf24)" : "none" }}>{enemy.icon}</span>
                                            <div style={{ width: 28, height: 2, background: "#2d1a1a", borderRadius: 2, marginTop: 1 }}>
                                                <div style={{ width: `${enemy.hp / enemy.maxHp * 100}%`, height: "100%", background: "#ef4444", borderRadius: 2 }} />
                                            </div>
                                        </div>
                                    )}
                                    {isPlayer && (
                                        <span style={{ fontSize: 19, filter: `drop-shadow(0 0 6px ${CLASSES[player.cls]?.color || "#fff"})` }}>{CLASSES[player.cls]?.icon}</span>
                                    )}
                                </div>
                            );
                        })}
...
function SidebarLog({ messages, logRef, movePlayer, openScreen }) {
    return (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", borderLeft: "1px solid #1e1e2e", minWidth: 0 }}>
            <div ref={logRef} style={{ flex: 1, overflowY: "auto", padding: "8px 10px", minHeight: 0 }}>
                {messages.map(m => <div key={m.id} style={{ color: m.color, fontSize: 11, marginBottom: 2, lineHeight: 1.5 }}>{m.text}</div>)}
            </div>
            <div style={{ flex: "0 0 auto", borderTop: "1px solid #1e1e2e", padding: "8px 10px", background: "#0d0d1a" }}>
                <p style={{ color: "#6b7280", fontSize: 10, marginBottom: 6 }}>CONTROLS (arrows/WASD)</p>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 4, marginBottom: 6 }}>
                    {[["", "⬆", ""], ["⬅", "⬇", "➡"]].map((row, ri) => (
                        <div key={ri} style={{ display: "contents" }}>
                            {row.map((k, ki) => (
                                <button key={ki} style={{ ...S.dpadBtn, visibility: k ? "visible" : "hidden" }}
                                    onClick={() => { if (k === "⬆") movePlayer(0, -1); if (k === "⬇") movePlayer(0, 1); if (k === "⬅") movePlayer(-1, 0); if (k === "➡") movePlayer(1, 0); }}>{k}</button>
                            ))}
                        </div>
                    ))}
                </div>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 3 }}>
                    {[["🎒", "Inv", "inventory"], ["📜", "Quests", "quests"], ["🌳", "Skills", "skilltree"], ["🔨", "Craft", "crafting"], ["🏴", "Factions", "factions"], ["📖", "Bestiary", "bestiary"], ["🏆", "Ach", "achievements"], ["🏠", "Home", "housing"]].map(([i, n, s]) => (
                        <button key={s} style={S.panelBtn} onClick={() => openScreen(s)}>
                            <span style={{ fontSize: 14 }}>{i}</span>
                            <span style={{ fontSize: 9, color: "#9ca3af" }}>{n}</span>
                        </button>
                    ))}
                </div>
                <div style={{ fontSize: 9, color: "#4b5563", marginTop: 6 }}>
                    <span>📦 chest ⬇ stairs ⛪ shrine ⚠ trap 🏪 shop 🏘 town 🧩 puzzle 🌀 portal ⛲ fountain ✨ secret 🔨 forge</span>
                </div>
            </div>
        </div>
    );
...
if (rootElement) {
    ReactDOM.createRoot(rootElement).render(React.createElement(DungeonQuest));
}
